...
/** Combine a family + weight with a base text style (e.g. an item's bold/italic). */
...
/** Like [applyThemeFont], but leaves the view's existing typeface/size untouched while the slot is at
 *  its defaults — so it layers on top of the app's own font handling (monospaced toggle / global size)
 *  instead of replacing it. Only an explicit family/weight/size override takes effect. */
fun TextView.applyThemeFontIfSet(slot: ThemeSlot, baseStyle: Int = Typeface.NORMAL) {
    val ctx = context
    val family = ctx.config.getFontFamily(slot.key)
    val weight = ctx.config.getFontWeight(slot.key)
    if (family.isNotEmpty() || weight > 0) {
        typeface = ctx.themeTypeface(family, weight, baseStyle)
    }
    val sizeSp = ctx.config.getFontSize(slot.key)
    if (sizeSp > 0) {
        setTextSize(TypedValue.COMPLEX_UNIT_SP, sizeSp.toFloat())
